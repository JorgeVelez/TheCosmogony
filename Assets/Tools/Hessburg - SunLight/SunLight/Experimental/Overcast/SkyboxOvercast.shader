...
// This is a modified version of the original procedural skybox shader of Unity 5
//
// The only difference to the original shader is the desaturation of the color of the sky based on the float _HessburgOvercast
// 

Shader "Hessburg/Experimental/Skybox-Procedural-Modified-Overcast" {
Properties {
	[KeywordEnum(None, Simple, High Quality)] _SunDisk ("Sun", Int) = 2
	_HessburgOvercast("Overcast", Range(0,1)) = 0.0 // 0.0 = clear - 1.0 = overcast
...
SubShader {
	...
	Pass {
		...
		uniform half _HessburgOvercast;
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			// convert and lerp color of the sky to greyscale to simulate overcast
			col = lerp(col, dot(col, float3(0.22, 0.707, 0.071)), _HessburgOvercast);
