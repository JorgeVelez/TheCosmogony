...
namespace TheVegetationEngine
{
    ...
    public class TVEGlobalVolume : StyledMonoBehaviour
    {
        ...
        [Tooltip("Volume data used for Colors elements rendering. In Dynamic mode, the buffer follows the Main Camera. In Quality mode, the buffer is rendered inside the Global Volume at the following resolutions: Low 512, Medium 1024, High 2048, Ultra 4096.")]
        public VolumeDataMode renderColorsData = VolumeDataMode.RenderedInVolumeMediumQuality;
        [Tooltip("Volume data used for Extras elements rendering. In Dynamic mode, the buffer follows the Main Camera. In Quality mode, the buffer is rendered inside the Global Volume at the following resolutions: Low 512, Medium 1024, High 2048, Ultra 4096.")]
        public VolumeDataMode renderExtrasData = VolumeDataMode.RenderedInVolumeMediumQuality;
        [Tooltip("Volume data used for Motion elements rendering. In Dynamic mode, the buffer follows the Main Camera. In Quality mode, the buffer is rendered inside the Global Volume at the following resolutions: Low 512, Medium 1024, High 2048, Ultra 4096.")]
        public VolumeDataMode renderMotionData = VolumeDataMode.RenderedInVolumeMediumQuality;
        [Tooltip("Volume data used for Interaction and Size elements rendering. In Dynamic mode, the buffer follows the Main Camera. In Quality mode, the buffer is rendered inside the Global Volume at the following resolutions: Low 512, Medium 1024, High 2048, Ultra 4096.")]
        ...
        void ExecuteRenderBuffers()
        {
            ...
            for (int i = 0; i < volumeDataSet.Count; i++)
            {
                ...
                if (volumeData.followMainCamera)
                {
                    if (mainCamera != null)
                    {
                        float gridX = 100f / 1024f;
                        float posX = Mathf.Round(mainCamera.transform.position.x / gridX) * gridX;

                        float gridZ = 100f / 1024f;
                        float posZ = Mathf.Round(mainCamera.transform.position.z / gridZ) * gridZ;

                        position = new Vector3(posX, mainCamera.transform.position.y, posZ);
                        scale = new Vector3(100, gameObject.transform.lossyScale.y, 100);
                    }
                }
